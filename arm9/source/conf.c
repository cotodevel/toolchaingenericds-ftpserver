...
__attribute__ ((optnone))
#endif
char * getMonthNameByNumericalMonth(int num) {
   switch(num){
      case(0):{
         return (char *)"Jan";
      }
...
__attribute__ ((optnone))
#endif
void trimStr(char *s)         // trim leading and trailing spaces+tabs
{
   int i,j,k, len;
   j=k=0;
   len = strlen(s);
...
__attribute__ ((optnone))
#endif
bool _Wifi_InitTGDSFTPServer(){
	DeInitWIFI();	//disable wifi card always
	bool useWIFI = true;
	return Wifi_InitTGDSFTPServer(useWIFI);
}
...
__attribute__ ((optnone))
#endif
bool Wifi_InitTGDSFTPServer(bool useFirmwareSettings) {
	...
	uint32 wifi_pass = Wifi_Init(WIFIINIT_OPTION_USELED|WIFIINIT_OPTION_USEHEAP_512);
