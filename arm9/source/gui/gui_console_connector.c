...
vramSetup * getProjectSpecificVRAMSetup(){
	return TGDSFTPSERVER_2DVRAM_SETUP();
}
...
//Definition that overrides the weaksymbol expected from toolchain to init console video subsystem
//1) VRAM Layout
vramSetup * TGDSFTPSERVER_2DVRAM_SETUP(){
	...
	//VRAM D: ARM7 (128 Ko!)
	vramSetupInst->vramBankSetupInst[VRAM_D_INDEX].vrambankCR = VRAM_D_0x06020000_ARM7;
	...
	vramSetupInst->vramBankSetupInst[VRAM_H_INDEX].vrambankCR = VRAM_H_LCDC_MODE; //SUB Engine: Console here
